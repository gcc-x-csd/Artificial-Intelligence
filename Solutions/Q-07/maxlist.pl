...
% 7. Write a Prolog program to implement maxlist(List,Max) so that Max is the greatest number in the list of numbers List using cut predicate.


% Define a predicate named `maxlist` that takes a list of numbers `List`
% and returns the maximum number in the list as the second argument `Max`.
